...
require get_template_directory() . '/admin/inc/post.php';
...
/**
 * quadra オプションTools デフォルト画像設定フィルター
 */
function quadra_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			'media_filename' => 'image_200x200.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_200x200.gif',
			'theme_option_keys' => array("['contents_builder'][3]['item_list'][0]['image']","['contents_builder'][3]['item_list'][1]['image']","['contents_builder'][3]['item_list'][2]['image']","['contents_builder'][3]['item_list'][3]['image']")
		),
		array(
			'media_filename' => 'image_1450x550.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x550.gif',
			'theme_option_keys' => array("index_header_type1_bg_image")
		),
		array(
			'media_filename' => 'image_1130x430.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1130x430.gif',
...
/**
 * quadra オプションTools サンプルカテゴリー
 */
function quadra_theme_options_tools_set_sample_categories( $dp_options ) {
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		...
		// 別々のアイコンをつけるため
		switch ($i) {
    	case 1: $icon = 'avn_blog'; break;
			case 2: $icon = 'cog'; break;
			case 3: $icon = 'lightbulb'; break;
			case 4: $icon = 'sphere'; break;
		}
  ...
  // ブログカテゴリー
	...
	foreach( $sample_categories as $sample_category ) {
		...
		// カテゴリーの場合のみ説明文追加
		$description = ($sample_category['taxonomy'] == 'category') ? __( 'Description will be displayed here.<br>Description will be displayed here.', 'tcd-w' ) : "";
		// カテゴリー追加
		$result = wp_insert_term(
			$sample_category['name'],
			$sample_category['taxonomy'],
			array(
				'description'=> isset( $sample_category['description'] ) ? $sample_category['description'] : $description,
		...
		if ( ! is_wp_error( $result ) && ! empty( $result['term_id'] ) && ! empty( $sample_category['metas'] ) ) {
			...
			// CBカテゴリー一覧のデフォルト値更新
			if($sample_category['taxonomy'] == 'category' && $sample_category['slug'] != 'category4')
				$dp_options['contents_builder'][0]['category'.$result['term_id']] = 1;
			
		}

		// CBタグクラウドのデフォルト値更新
		if ( ! is_wp_error( $result ) && ! empty( $result['term_id'] ) && $sample_category['taxonomy'] == 'post_tag' ) {
			$dp_options['contents_builder'][2]['tag'.$result['term_id']] = 1;
		}
...
/**
 * quadra オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function quadra_theme_options_tools_set_sample_posts( $dp_options ) {

	// 各詳細ページのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_750x420.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_750x420.gif'
	), true );
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample blog', 'tcd-w' ) . $i,
			'post_content' => __( 'Sample text. Sample text. Sample text. Sample text.', 'tcd-w' ),
			'post_name' => 'sample-blog' . $i,
			'post_status' => 'publish',
			'post_type' => 'blog',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $media_id,
			'taxonomies' => array(
				'blog_category' => array('blog_category1','blog_category2','blog_category3','blog_category4')
			),
			'metas' => array(
				'recommend_post' => 'on',
				'blog_content_type' => 'type1',
				'blog_use_overlay' => 1,
				'blog_overlay_color' => '#000000',
				'blog_overlay_opacity' => '0.3',
			)
		);
	}
...
/**
 * quadra オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function quadra_theme_options_tools_set_sample_menus( $dp_options ) {
...
/**
 * quadra オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function quadra_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
				'headline' => __('Category', 'tcd-w'),
				'category_type' => 'blog_category',
				'exclude_cat_num' => '',
				'hierarchical' => '',
			)
		),
		...
		// タブ記事
		array(
			'class_name' => 'tab_post_list_widget',
			'params' => array(
				'title1' => __('Recent post', 'tcd-w'),
				'title2' => __('Recommend post', 'tcd-w'),
				'post_num1' => '4',
				'post_num2' => '4',
				'post_order1' => 'date',
				'post_order2' => 'rand',
				'post_type1' => 'recent_post',
				'post_type2' => 'recommend_post',
				'show_tab1' => 1,
				'show_tab2' => 1,
			)
		),
 ...
 * quadra テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'quadra_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'quadra_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'quadra_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'quadra_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'quadra_theme_options_tools_set_sample_widgets', 10, 1 );
