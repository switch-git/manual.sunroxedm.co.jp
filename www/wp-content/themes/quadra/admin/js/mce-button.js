			...
			if (k !== 'pulldown_title' && typeof tcdQuicktagsL10n[k] === 'object') {
				menus.push({
					...
					onclick: function() {
						...
						if (tcdQuicktagsL10n[k].tagStart && tcdQuicktagsL10n[k].tagEnd && selectionContent) {
							editor.insertContent(tcdQuicktagsL10n[k].tagStart + selectionContent + tcdQuicktagsL10n[k].tagEnd);
						} else if (tcdQuicktagsL10n[k].tag) {
							editor.insertContent(selectionContent + tcdQuicktagsL10n[k].tag);
						}
					}
				});
			}
