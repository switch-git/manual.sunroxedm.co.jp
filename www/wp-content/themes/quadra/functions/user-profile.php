...
?>
  ...
  <h4 class="headline"><?php _e( 'Display setting', 'tcd-w' ); ?></h4>
  <div class="input_field" style="margin-bottom:7px;">
   <input name="show_author" type="checkbox" value="1" <?php checked( $user->show_author, 1 ); ?>> <?php _e( 'Show author profile at single page', 'tcd-w'); ?>
	</div><!-- END input_field -->
	<div class="input_field">
	 <input name="show_author_blog" type="checkbox" value="1" <?php checked( $user->show_author_blog, 1 ); ?>> <?php printf(__('Show author profile at single %s page', 'tcd-w'), $blog_label); ?>
  </div><!-- END input_field -->
...
function update_tcdw_profile_fields( $user_id ) {
	...
	if ( user_can( $user_id, 'edit_posts' ) ) {
		$meta_keys = array(
			...
			'show_author_blog' => $_POST['show_author_blog']
		);
	} else {
		$meta_keys = array();
	}
