...
function seo_title( $title, $sep ) {
  ...
  $options = get_design_plus_option();
	...
	// 投稿アーカイブ
		...
		if($options['post_archive_meta_title']){
			$title = $options['post_archive_meta_title'];
		}else{
			$title = $options['post_label'] .  ' | ' . $site_title;
		}
	...
	// 各タクソノミーアーカイブ
    	 ...
    	 $title = $term_meta['meta_title'];
			 ...
			 $title = sprintf( __( 'Post list for %s', 'tcd-w' ), $term_name );
		 }
	...
	// 著者ページ
  	...
  	$title = $curauth->display_name . ' | ' . $site_title;
...
function get_seo_description() {
	...
	// 投稿アーカイブ
		...
		if ( $options['post_archive_meta_description'] ) {
			$trim_content = $options['post_archive_meta_description'];
			$trim_content = str_replace( array( "\r\n", "\r", "\n" ), '', $trim_content );
			return esc_html( $trim_content );
		}else{
			return esc_html( $site_description );
