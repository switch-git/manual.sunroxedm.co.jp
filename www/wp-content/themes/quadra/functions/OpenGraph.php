...
class OpenGraph implements Iterator
{
   ...
   */
	static public function fetch($URI) {
		...
		// Load cache
		if ( $can_cache ) {
			$cache_key = 'tcd_opengraph_' . md5( $URI );
			$response  = get_transient( $cache_key );
		}

		// Disable cache or cache expired
		if ( false === $response ) {
			// Get html from url
		...
		if ( $can_cache ) {
			...
			if ( function_exists( 'apply_filters' ) ) {
				$cache_seconds = apply_filters( 'open_graph_fetch_cache_seconds', $cache_seconds );
			}
